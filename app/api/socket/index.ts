...
export function setupWebSocketServer(httpServer: HttpServer) {
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("authenticate", (userId: string) => {
      ...
      console.log(
        `[${new Date().toISOString()}] User ${userId} authenticated with socket ${
          socket.id
        }`
      );
    });
    ...
    socket.on("join", async ({ userId, roomId, roomType }) => {
      socket.join(roomId);
      console.log(
        `[${new Date().toISOString()}] User ${userId} joined ${roomType} ${roomId}`
      );
      ...
      console.log(
        `[${new Date().toISOString()}] Sent room history to user ${userId} for ${roomType} ${roomId}`
      );
    });
    ...
    socket.on("leave", ({ userId, roomId, roomType }) => {
      socket.leave(roomId);
      console.log(
        `[${new Date().toISOString()}] User ${userId} left ${roomType} ${roomId}`
      );
    });

    socket.on(
      "sendMessage",
      async ({ userId, roomId, roomType, content, language }) => {
        console.log(
          `[${new Date().toISOString()}] Received message from user ${userId} in ${roomType} ${roomId}`
        );
        try {
          ...
          console.log(
            `[${new Date().toISOString()}] Saved message from user ${userId} in ${roomType} ${roomId}`
          );
          ...
          if (roomType === "group") {
            ...
            for (const member of group?.members ?? []) {
              ...
              console.log(
                `[${new Date().toISOString()}] Translated message for user ${
                  member.user.id
                } in group ${roomId}`
              );
              ...
              if (memberSockets) {
                memberSockets.forEach((socketId) => {
                  ...
                  console.log(
                    `[${new Date().toISOString()}] Sent translated message to user ${
                      member.user.id
                    } socket ${socketId}`
                  );
                });
            ...
            console.log(
              `[${new Date().toISOString()}] Generated AI response for user ${userId} in AI chat ${roomId} and the language being ${language}`
            );
            ...
            console.log(
              `[${new Date().toISOString()}] Sent AI response to user ${userId} in AI chat ${roomId}`
            );
          }
        } catch (error) {
          console.error(
    ...
    socket.on("disconnect", () => {
      ...
      userSockets.forEach((sockets, userId) => {
        if (sockets.has(socket.id)) {
          ...
          console.log(
            `[${new Date().toISOString()}] Removed socket ${
              socket.id
            } for user ${userId}`
          );
          if (sockets.size === 0) {
            ...
            console.log(
              `[${new Date().toISOString()}] Removed all sockets for user ${userId}`
            );
          }
          return;
        }
...
async function getMessages(roomId: string, roomType: string) {
  console.log(
    `[${new Date().toISOString()}] Fetching messages for ${roomType} ${roomId}`
  );
...
async function saveMessage(
  ...
  console.log(
    `[${new Date().toISOString()}] Saving message for user ${userId} in ${roomType} ${roomId}`
  );
  let savedMessage;
  if (roomType === "group") {
  ...
  console.log(
    `[${new Date().toISOString()}] Saved message with ID ${
      savedMessage?.id
    } for user ${userId} in ${roomType} ${roomId}`
  );
  return savedMessage;
}
